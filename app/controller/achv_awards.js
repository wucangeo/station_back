...
class AchvAwardsController extends Controller {
  async list() {
    ...
    const result = await service.achvAwards.list(query);
    ctx.body = result;
  }
  async get() {
    const { ctx, service } = this;
    const { validator } = this.app;
    let result = await service.achvAwards.get(ctx.params.data_id);
    ctx.body = result;
  }
  async create() {
    const { ctx, service } = this;
    ...
    let result = await ctx.service.achvAwards.create(item, user_id);
    ctx.body = result;
  }
  async update() {
    const { ctx, service } = this;
    ...
    var result = await service.achvAwards.update(
  ...
  async delete() {
    const { ctx, service } = this;
    let result = await service.achvAwards.delete(ctx.request.body.ids);

    ctx.body = result;
  }
}
module.exports = AchvAwardsController;
