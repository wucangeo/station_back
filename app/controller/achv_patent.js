...
class AchvPatentController extends Controller {
  async list() {
    ...
    const result = await service.achvPatent.list(query);
    ctx.body = result;
  }
  async get() {
    const { ctx, service } = this;
    const { validator } = this.app;
    let result = await service.achvPatent.get(ctx.params.data_id);
    ctx.body = result;
  }
  async create() {
    const { ctx, service } = this;
    ...
    let result = await ctx.service.achvPatent.create(item, user_id);
    ctx.body = result;
  }
  async update() {
    const { ctx, service } = this;
    ...
    var result = await service.achvPatent.update(
  ...
  async delete() {
    const { ctx, service } = this;
    let result = await service.achvPatent.delete(ctx.request.body.ids);

    ctx.body = result;
  }
}
module.exports = AchvPatentController;
