...
class News extends Service {
  ...
  async get(data_id) {
    ...
    //查询
    var news = await ctx.model.News.findOne(query);
  ...
  async create(item, user_id = 0) {
    ...
    //查询
    let data_created = await ctx.model.News.create(item);
    if (!data_created) {
      result.code = 0;
  ...
  async update({ data_id, updates }, user_id = 0) {
    ...
    //data_id有效性验证
    const news = await ctx.model.News.findById(data_id);
    if (!news) {
      result.code = 0;
    ...
    //查询
    let data_updated = await news.update(updates);
  ...
  async delete(ids) {
    ...
    //查询
    const data_deleted = await ctx.model.News.destroy({
